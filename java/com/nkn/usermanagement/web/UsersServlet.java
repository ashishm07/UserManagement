...
//			session.invalidate();
	...
	Cookie cookie[] = request.getCookies();for(
	Cookie c:cookie)
	{
...
//			out.print(name + " does not exist");
	...
	Cookie cookie1 = new Cookie("uname",
			"");cookie1.setMaxAge(0);response.addCookie(cookie1);response.sendRedirect("userLogin.jsp");
	}

	if(request.getParameter("Add")!=null){
	// insertUser(request, response);
	response.sendRedirect("user-form.jsp");}else if(request.getParameter("Add User")!=null){insertUser(request,response);}else if(request.getParameter("Delete")!=null){response.sendRedirect("delUser.jsp");}else if(request.getParameter("Delete User")!=null){deleteUser(request,response);}else if(request.getParameter("Update")!=null){response.sendRedirect("updateUser.jsp");}else if(request.getParameter("Update User")!=null){updateUser(request,response);}else if(request.getParameter("List All Users")!=null){listUser(request,response);}
